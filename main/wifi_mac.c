...
{
    ESP_LOGI(TAG, "---- MAC extra register dump ----");
    ...
    ESP_LOGI(TAG, "WIFI_MAC_ADDR0_LOW    = 0x%08lx", WIFI_MAC_ADDR0_LOW);
    ESP_LOGI(TAG, "WIFI_MAC_ADDR0_HIGH   = 0x%08lx", WIFI_MAC_ADDR0_HIGH);
    ESP_LOGI(TAG, "WIFI_MAC_ADDR0_CTRL   = 0x%08lx", WIFI_MAC_ADDR0_CTRL);
    ESP_LOGI(TAG, "WIFI_BSSID0_LOW       = 0x%08lx", WIFI_BSSID0_LOW);
    ESP_LOGI(TAG, "WIFI_BSSID0_HIGH      = 0x%08lx", WIFI_BSSID0_HIGH);
    ESP_LOGI(TAG, "WIFI_BSSID0_MASK_LOW  = 0x%08lx", WIFI_BSSID0_MASK_LOW);
    ESP_LOGI(TAG, "WIFI_BSSID0_MASK_HIGH = 0x%08lx", WIFI_BSSID0_MASK_HIGH);
    ESP_LOGI(TAG, "---------------------------------");
}

void wifi_mac_rx_enable_experimental(void)
{
    ESP_LOGI(TAG, "experimental MAC RX enable start");
    ...
    ESP_LOGI(TAG, "MAC_CTRL: before=0x%08lx after=0x%08lx", mac_ctrl_before, mac_ctrl_after);
    ...
    ESP_LOGI(TAG, "experimental MAC RX enable done");
}
...
void wifi_mac_rx_bit_probe_task(void *arg)
{
    ...
    ESP_LOGI(TAG, "RX bit probe task started");
    ...
    ESP_LOGI(TAG, "original POLICY0=0x%08lx", original_policy0);
    ESP_LOGI(TAG, "original POLICY1=0x%08lx", original_policy1);
    ESP_LOGI(TAG, "original CTRL   =0x%08lx", original_ctrl);

    for (int bit = 0; bit < 32; bit++) {
        uint32_t mask = 1UL << bit;

        ESP_LOGI(TAG, "PROBE POLICY0 bit %d mask=0x%08lx", bit, mask);
    ...
    for (int bit = 0; bit < 32; bit++) {
        ...
        ESP_LOGI(TAG, "PROBE POLICY1 bit %d mask=0x%08lx", bit, mask);
    ...
    for (int bit = 0; bit < 16; bit++) {
        ...
        ESP_LOGI(TAG, "PROBE CTRL clear bit %d mask=0x%08lx", bit, mask);
    ...
    ESP_LOGI(TAG, "RX bit probe finished");
    vTaskDelete(NULL);
}
